...
const FinanceHome = ({ categories, articles, tags, users, totalCount }) => {
  ...
  const loadInitialArticles = useCallback(async () => {
    const initialArticles = await getArticlesByPathSegment("finance", 0, loadedArticles); // Pass pathSegment to the getArticles function
  ...
  const handleLoadMore = async () => {
    ...
    const limit = 5;
    const additionalArticles = await getArticlesByPathSegment("finance", skip, limit); // Fetch the next 10 articles (adjust the limit as needed)
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    const [subcategories, categories, articles, tags, users, {totalCount}] = await Promise.all([
      ...
      import('../../../sanity/query functions/query').then((module) => module.getFeaturedArticlesByPathSegment('finance',  0, 2)),
      ...
      import('../../../sanity/query functions/query').then((module) => module.getArticlesByPathSegment('finance',  0, 2)),
    ]);
