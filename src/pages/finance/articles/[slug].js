...
const DynamicArticlePage = ({ article, related, ogImageUrl }) => {
  const { title, body, createdAt, image, lastUpdated, metaDescription, metaTitle, postedBy: { image: userImage, username }, slug: { current: slug }, _createdAt, _id, _updatedAt } = article;
    ...
    <Layout  >
      <Card elevation={0} sx={{}}>
        ...
        <Box sx={{ zIndex: 3, width: '100%' }}>
          <Typography sx={{ fontSize: { xs: '2rem', md:'3rem' }, fontWeight: 'bold', color: indexFontColor, width: '100%', textAlign: 'center' }} variant="h6" component="div">
            {title}
          </Typography>
        </Box>
