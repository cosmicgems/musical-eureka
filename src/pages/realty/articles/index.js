...
const RealtyArticles = ({ articles, totalCount }) => {
  ...
  const loadInitialArticles = useCallback(async () => {
    const initialArticles = await getArticlesByPathSegment('realty', 0, loadedArticles);
  ...
  const handleLoadMore = async () => {
    ...
    const limit = 5;
    const additionalArticles = await getArticlesByPathSegment('realty', skip, limit); // Fetch the next 5 articles (adjust the limit as needed)
...
export async function getStaticProps() {
  try {
    const { articles, totalCount } = await getArticlesByPathSegment('realty', 0, 5);
