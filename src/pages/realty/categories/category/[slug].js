...
function SlugPage({ articles, slug, name, totalCount }) {
  ...
  const loadInitialArticles = useCallback(async () => {
    const initialArticles = await getArticlesBySubcategory("realty",slug, 0, loadedCount); 
  ...
  const handleLoadMore = async () => {
    ...
    const additionalArticles = await getArticlesBySubcategory("realty",slug, skip, limit);
...
export async function getStaticProps({ params }) {
  const { slug} = params;
  const {articles, totalCount} = await getArticlesBySubcategory("realty",slug);
