...
const IntelligenceArticles = ({ articles, totalCount }) => {
  ...
  const loadInitialArticles = useCallback(async () => {
    const initialArticles = await getArticlesByPathSegment('intelligence', 0, loadedArticles);
  ...
  const handleLoadMore = async () => {
    ...
    const limit = 5;
    const additionalArticles = await getArticlesByPathSegment('intelligence', skip, limit); // Fetch the next 5 articles (adjust the limit as needed)
...
export async function getStaticProps() {
  try {
    const { articles, totalCount } = await getArticlesByPathSegment('intelligence', 0, 5);
...
export default IntelligenceArticles;
