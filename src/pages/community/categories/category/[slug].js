...
function SlugPage({ articles, slug, name, totalCount }) {
  ...
  const loadInitialArticles = useCallback(async () => {
    const initialArticles = await getArticlesBySubcategory("community",slug, 0, loadedCount); 
  ...
  const handleLoadMore = async () => {
    ...
    const additionalArticles = await getArticlesBySubcategory("community",slug, skip, limit);
...
export async function getStaticProps({ params }) {
  const { slug} = params;
  const {articles, totalCount} = await getArticlesBySubcategory("community",slug);
