...
const ArtArticles = ({ articles, totalCount }) => {
  ...
  const loadInitialArticles = useCallback(async () => {
    const initialArticles = await getArticlesByPathSegment('art', 0, loadedArticles);
  ...
  const handleLoadMore = async () => {
    ...
    const limit = 5;
    const additionalArticles = await getArticlesByPathSegment('art', skip, limit); // Fetch the next 5 articles (adjust the limit as needed)
...
export async function getStaticProps() {
  try {
    const { articles, totalCount } = await getArticlesByPathSegment('art', 0, 5);
