...
function SlugPage({ articles, slug, name, totalCount }) {
  ...
  const loadInitialArticles = useCallback(async () => {
    const initialArticles = await getArticlesBySubcategory("art",slug, 0, loadedCount); 
  ...
  const handleLoadMore = async () => {
    ...
    const additionalArticles = await getArticlesBySubcategory("art",slug, skip, limit);
...
export async function getStaticProps({ params }) {
  const { slug} = params;
  const {articles, totalCount} = await getArticlesBySubcategory("art",slug);
