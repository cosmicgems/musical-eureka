...
const TechnologyArticles = ({ articles, totalCount }) => {
  ...
  const handleLoadMore = async () => {
    ...
    const limit = 5;
    const additionalArticles = await getArticlesByPathSegment('technology', skip, limit); // Fetch the next 5 articles (adjust the limit as needed)
...
export async function getStaticProps() {
  try {
    const { articles, totalCount } = await getArticlesByPathSegment('technology', 0, 5);
